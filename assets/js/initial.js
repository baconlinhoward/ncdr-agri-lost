$(document).ready(function () {
        ...
        if (target == "overview") {
            ...
            setTimeout(function () {
                ...
                var layerIndex = 0,
                    ...
                    regionLayer = L.geoJSON(data.taiwanLayer, {
                        interactive: false,
                        ...
                        onEachFeature: function (feature, layer) {
                            feature.properties["layerIndex"] = layerIndex
                ...
                mapInterval = setInterval(() => {
                    ...
                    regionLayer.eachLayer(function (layer) {
                        var thisLayerIndex = layer.feature.properties.layerIndex
                        if ((startAnimationIndex % layerIndex) === thisLayerIndex) {
                            layer.openTooltip()
                        }
                    })
          ...
          <table class="table table-sm table-hover text-center w-100">
            ...
            $("#caseDetailedPageSelector").change(function(){
                if ($(this).val() === "chart") {
                    $("#caseDetailedPagePowerBiChart").css("display", "block")
                    if (targetData.analysisChart === undefined || targetData.analysisChart === "") {
                        alert("此調查事件無分析圖！")
                        $("#caseDetailedPageSelector").val("text").trigger("change")
                    }
                    if ($("#caseDetailedPagePowerBiChart").attr("src") === "" && targetData.analysisChart !== undefined && targetData.analysisChart !== "") {
                        $("#caseDetailedPagePowerBiChart").attr("src", targetData.analysisChart)
                    }
                } else {
                    $("#caseDetailedPagePowerBiChart").css("display", "none")
                }
            })
            setTimeout(function () {
                map = initialMap({
    ...
    // initial
    $('#atrr-info-modal').on('hidden.bs.modal', function () {
		$("#atrr-info-modal .modal-body").html("");
	})
        ...
        if (modalTrigBool) {
            switch (parameterList.type) {
                case "image":
                    ...
                    break
                case "iframe":
                    html = `<iframe class="w-100 h-500 border-0" src="${parameterList.url}"></iframe>`
                    break
            }
            modal_open({
...
function casePageTemplate(data) {
    ...
    var html = `
        ...
        <div class="container" data-aos="fade-up" style="max-width: unset">
            ...
            <div class="col-lg-9">
                <div class="row">
                <button style="z-index: 9999; top: calc( 100vh - 167px ); left: calc( 37.5% - 98px);" type="button" id="caseDetailedPageInitial" class="position-fixed btn btn-success shadow-sm font-weight-bold pt-1 pb-1">查看事件紀實(共${data.pageData.page.length}頁)<i class="icofont-simple-down ml-2"></i></button>
                <div class="col-lg-12" style="padding-bottom: 30px !important;">
                    <h4 class="mb-1 font-weight-bold">
                        <span class="mr-3" style="vertical-align: middle;"><i class="icofont-info-square mr-2"></i>${data.year + "年「" + data.name}」農損事件</span>
                        <div class="form-group float-right">
                            <div class="input-group input-group-sm">
                                <div class="input-group-prepend">
                                    <span class="input-group-text">內容頁面</span>
                                </div>
                                <select class="form-control form-control-sm" id="caseDetailedPageSelector">
                                    <option value="text">事件資訊</option>
                                    <option value="chart">事件分析圖</option>
                                </select>
                            </div>
                        </div>
                    </h4>
                    <iframe id="caseDetailedPagePowerBiChart" class="w-100 border-0" style="height: calc( 100vh - 165px ); display: none;" src=""></iframe>
                </div>
                <div class="col-lg-3 col-md-6">
                ...
                <div class="col-lg-5 overflow-auto mt-3" style="height: calc( 100vh - 360px );">
                    <ul>
                        <li>
                            事件資訊
                            <ul>
                ...
                <div class="col-lg-7 overflow-auto mt-3" style="height: calc( 100vh - 360px );">
                    <ul>
                    <li>影響區域表</li>
            ...
            <div class="col-lg-3">
                <div class="w-100 rounded" id="map" style="height: calc( 100vh - 135px )"></div>
            </div>
            </div>
        </div>
        </div>`
...
function caseDetailedPageTemplateCreate(info, pageIndex) {
    ...
    if (info.type === "map") {
        ...
        mapHtml = `<div id="caseDetailedPageMap${pageIndex}" class="w-100" style="height: 250px;" points='${JSON.stringify(info.coordinates)}'></div>`
    }
    var html = {
        text: `
            ...
            if ($('#caseDetailedPageMap' + index).length) {
                setTimeout(function () {
                    var map = initialMap({
                        ...
                        pointsArray = JSON.parse($('#caseDetailedPageMap' + index).attr('points')),
                        geoJson = {
                            "type": "FeatureCollection",
                            "features": []
                        }
                    pointsArray.forEach(each => {
                        geoJson.features.push({
                            "type": "Feature",
                            "geometry": {
                                "type": "Point",
                                "coordinates": [Number(each[1]), Number(each[0])]
                            }
                        })
                    })
                    var thisLayer = L.geoJSON(geoJson).addTo(map).on('click', function (e) {
                        var url_string = "https://maps.google.com/maps?q=&layer=c&cbll=" + e.layer.feature.geometry.coordinates[1] + "," + e.layer.feature.geometry.coordinates[0] + "&cbp=12,270";
